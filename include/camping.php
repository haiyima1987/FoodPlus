...
//    $query_check_reserved = "SELECT id_number FROM reserved_account WHERE id_number in (" . $reserve_account;
    ...
    $query_check_limit = "SELECT id_number FROM reservation WHERE id_number = " . $reserve_account;
    $account_limit = $db->selectUpdateData($query_check_limit)->fetchAll(PDO::FETCH_ASSOC);

    if (count($person_list) == $person_count) {
        if (count($account_limit) < 1) {
            ...
            echo '<p>One Account can reserve maximum 1 spots</p>';
            echo '</div>';
        }
    } else {
        echo '<div class="alert alert-danger">';
...
if (!$reservation_complete) {
    ...
    <div class="content col-xs-7 col-xs-offset-1">
        ...
        $db = new DbcHelper(DB_HOST, DB_USER, DB_PASSWORD, DB_NAME);
        $query_sites = "SELECT DISTINCT site FROM tent";
